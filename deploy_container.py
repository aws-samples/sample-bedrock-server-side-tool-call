...
def wait(seconds):
    """Block for the specified duration using an event-based wait."""
    event = threading.Event()
    event.wait(timeout=seconds)


def poll_until_ready(client, runtime_id, interval, max_attempts):
    """Poll runtime status until READY or max attempts reached."""
...
def main():
    ...
    # Build & push
    subprocess.run(["docker", "build", "-t", f"{REPO_NAME}:latest", "."], check=True)
    ...
    try:
        ...
        print(f"   Waiting {IAM_PROPAGATION_DELAY}s for IAM propagation...")
        wait(IAM_PROPAGATION_DELAY)
    ...
    status = poll_until_ready(ctrl, runtime_id, POLL_INTERVAL, MAX_POLL_ATTEMPTS)
    ...
    for attempt in range(MAX_RETRIES):
        try:
            if attempt > 0:
                print(f"   Retry {attempt}/{MAX_RETRIES - 1} in {RETRY_DELAY}s...")
                wait(RETRY_DELAY)
